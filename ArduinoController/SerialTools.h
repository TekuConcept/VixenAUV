...
public:
  ...
  static float readFloat() {
    float result = 0;
    readData((uint8_t*)&result, sizeof(float));
    return result;
  }

  static double readDouble() {
    double result = 0;
    readData((uint8_t*)&result, sizeof(double));
    return result;
  }

  static int32_t readInt() {
    int result = 0;
    readData((uint8_t*)&result, sizeof(int32_t));
    return result;
  }

  static uint32_t readUInt() {
    uint32_t result = 0;
    readData((uint8_t*)&result, sizeof(uint32_t));
    return result;
  }

  static int16_t readShort() {
    int16_t result = 0;
    readData((uint8_t*)&result, sizeof(int16_t));
    return result;
  }

  static uint16_t readUShort() {
    uint16_t result = 0;
    readData((uint8_t*)&result, sizeof(uint16_t));
    return result;
  }

  static void readData(uint8_t* ptr, size_t size) {
   ...
   * out buffer.
   */
  static void writeString(const char* data, size_t size) {
    for(int i = 0; i < size; i++) {
      Serial.print(data[i]);
    }
    Serial.print('\0');
  ...
  static void writeData(char* ptr, size_t size) {
    for(int i = 0; i < size; i++){
      #ifdef DEBUG
  ...
  static void printHex(uint8_t c) {
    printSymbol((uint8_t)((c&0xF0) >> 4));
    printSymbol((uint8_t)(c&0x0F));
  }

private:
  SerialEx(){}
  static void printSymbol(uint8_t val) {
    char res = '0';
    if(val < 10)
      Serial.print(val);
    else
